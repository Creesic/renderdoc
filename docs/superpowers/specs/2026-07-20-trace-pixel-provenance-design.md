...
- The public replay API (`ActionDescription.copySource`/`copyDestination`) exposes only source/destination **resource + subresource**, not a sub-rectangle offset. The tool compares resource dimensions at their respective mips before crossing the hop and stops with `stopped_reason: "partial_rect_copy_unsupported"` when they don't match, rather than silently returning a pixel from the wrong location — same honesty convention as `diff_shader_invocations`' truncation reporting and the README's existing "returns `available: false` instead of silently pretending defaults" pattern. This dimension check only catches partial-rect copies/blits that also change the overall resource size; an offset partial-rect copy between two equally-sized resources (e.g. `CopySubresourceRegion` copying only a sub-rect into an equally-sized destination) is indistinguishable from a full-surface copy through this API, so it is silently assumed aligned — a real, undetectable-from-here limitation, not one this check guards against.
